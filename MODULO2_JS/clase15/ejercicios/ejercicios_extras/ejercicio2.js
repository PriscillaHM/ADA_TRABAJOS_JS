// Clasificación de palabras según su longitud 
// Tienes una lista de palabras en una oración. Debes:
// 1. Dividir la oración en palabras individuales.
// 2. Clasificar las palabras en dos categorías:
// ▪ Cortas (menos de 5 letras)
// ▪ Largas (5 letras o más)
// 3. Mostrar ambas listas de palabras clasificadas.
// 4. La oración es: "JavaScript es un lenguaje poderoso y versátil".
let palabras = 'JavaScript es un lenguaje poderoso y versátil';
console.log('Oracion incial: ', palabras);

// 1. Dividir la oración en palabras individuales.
let palabrasDivididas = palabras.split(' ');
console.log('Oracion dividida: ', palabrasDivididas)

// 2. Clasificar las palabras en dos categorías:
// ▪ Cortas (menos de 5 letras)
// ▪ Largas (5 letras o más)
let palabrasCortas = [];
let palabrasLargas = [];

for(let i = 0; i < palabrasDivididas.length; i++){
    palabrasDivididas[i].length < 5 ? palabrasCortas.push(palabrasDivididas[i]) : palabrasLargas.push(palabrasDivididas[i]);
}

// 3. Mostrar ambas listas de palabras clasificadas.
console.log('Lista de palabras cortas: ', palabrasCortas);
console.log('Lista de palabras largas: ',palabrasLargas);